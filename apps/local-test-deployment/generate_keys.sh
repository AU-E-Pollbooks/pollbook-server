...
openssl req -x509 -newkey rsa -keyout server0/private_key.pem -out server0/checkin_certificate.csr -sha256 -days 365 -nodes -subj "/C=US/ST=Georgia/L=Augusta/O=AU/OU=SCCS/CN=Check In Server"
#sign the certificate for server 0 using the CA
openssl x509 -req -in server0/checkin_certificate.csr -CA server0/ca/ca_cert.pem -CAkey server0/ca/ca_key.pem -out server0/checkin_certificate.pem
#CSR is no longer needed, discard
rm server0/checkin_certificate.csr

#generate the certificate request for server 1
openssl req -x509 -newkey rsa -keyout server1/private_key.pem -out server1/id_certificate.csr -sha256 -days 365 -nodes -subj "/C=US/ST=Georgia/L=Augusta/O=AU/OU=SCCS/CN=Voter ID Server"
#sign the certificate for server 1 using the CA
openssl x509 -req -in server1/id_certificate.csr -CA server1/ca/ca_cert.pem -CAkey server1/ca/ca_key.pem -out server1/id_certificate.pem
#CSR is no longer needed, discard
...
for num in {0..3}; do
    openssl req -x509 -newkey rsa -keyout client${num}/private_key.pem -out client${num}/certificate.csr -sha256 -days 365 -nodes -subj "/C=US/ST=Georgia/L=Augusta/O=AU/OU=SCCS/CN=Client ${num}"
    openssl x509 -req -in client${num}/certificate.csr -CA client${num}/ca/ca_cert.pem -CAkey client${num}/ca/ca_key.pem -out client${num}/certificate.pem
